...
import { getRecommendations, getTrackFeatures, type RecommendResponse, type RecommendMethod, type AudioFeatures } from '../api/client';
...
import { Loader2, AlertCircle, LayoutGrid, MapPin, ArrowLeft, SlidersHorizontal, Filter } from 'lucide-react';
...
export function ResultsPage() {
  ...
  const [q4Only, setQ4Only] = useState(false);
  ...
  async function handleGetRecommendations() {
    ...
    try {
      const data = await getRecommendations({
        ...
        method,
        alpha,
        emotional_filter: q4Only ? 'Q4' : undefined,
      });
      setResult(data);
    } catch {
          ...
          <aside className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-rose-100 p-5 shadow-sm space-y-4">
              ...
              {method === 'hybrid' && (
                <div>
                  <div className="flex justify-between text-xs text-stone-500 mb-1">
                    ...
                    <span className="font-medium text-stone-700">α = {alpha.toFixed(2)}</span>
                    <span>Intimacy</span>
                  </div>
                  <input
                    type="range"
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <div
                  onClick={() => setQ4Only(v => !v)}
                  className={`w-9 h-5 rounded-full transition-colors relative ${q4Only ? 'bg-rose-500' : 'bg-stone-200'}`}
                >
                  <span className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${q4Only ? 'translate-x-4' : 'translate-x-0.5'}`} />
                </div>
                <span className="text-xs text-stone-600 flex items-center gap-1">
                  <Filter size={11} /> Intimate zone only (Q4)
                </span>
              </label>
